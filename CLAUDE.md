...
- **`editor/DbfSearchController`** — the Cmd-F search bar: a debounced match pass over the whole
  ...
  via a `searchSeq` generation counter. To avoid the memory cliff of one coordinate per matching cell
  on a broad query over a huge file, the scan (`DbfTableSearch.scan`) returns only an `O(rows)`
  aggregate — per-row match counts (`int[]`) + total + the anchor-relative current cell/rank — and the
  per-cell highlight is recomputed **live** in `isMatch` against a cached `Pattern`; prev/next navigate
  via the `matchRows` `BitSet`. A single committed cell edit patches that aggregate in place
  (`updateCellMatch`: the edited row's count delta of ±1 keeps `total`/rank exact) instead of rescanning
  the whole table — unless a scan is still in flight (`searchSeq != appliedSeq`), when it falls back to a
  rescan. The pure matching logic lives in `ui/DbfTableSearch` (Swing-free, tested); its background
  overloads poll a cancellation check and return `null` when superseded. See
  `notes/search-wide-query-memory.md`.
